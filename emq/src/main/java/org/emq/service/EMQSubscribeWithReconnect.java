...
public class EMQSubscribeWithReconnect {
	String userName; // 非必须
	String password; // 非必须
	String host;
	String topic;
	...
	int outTime = 10;
	int keepTime = 20;
	public EMQSubscribeWithReconnect(String userName,String password,String host,String topic,String clientId) {
	...
	public static void main(String[] args) {
		...
		String topic = "/World";
		String topic2 = "/World2";
		String clientId = UUID.randomUUID().toString();
		String clientId2 = UUID.randomUUID().toString();
		EMQSubscribeWithReconnect testEmqReConnect = new EMQSubscribeWithReconnect(userName,password,host,topic,clientId);
		testEmqReConnect.subscribeWithReconnect();
		EMQSubscribeWithReconnect testEmqReConnect2 = new EMQSubscribeWithReconnect(userName,password,host,topic2,clientId2);
		testEmqReConnect2.subscribeWithReconnect();
	}
	 ...
	 */
	public void subscribeWithReconnect() {
		while(true) {
...
//            option.setWill(topic, "close".getBytes(), 2, true);
	...
	private class PushCallBack implements MqttCallback {
		...
		public void messageArrived(String topic, MqttMessage message) throws Exception {
			...
			System.out.println("客户端"+clientId+":"+"接收消息内容 : " + new String(message.getPayload()));
		}
